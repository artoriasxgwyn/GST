  ...
  <section class="pricing">
      ...
      <div class="carousel-viewport" ref="planViewport">
        <div class="plans-track" :style="{ transform: `translateX(${planTrackOffset}px)` }">
          <div v-for="(plan, i) in plans" :key="plan.name" class="plan-card"
            ...
            <button :class="plan.featured ? 'btn-primary' : 'btn-outline'" @click="emit('toggle-form', plan)">
              <i
                :class="['bi', plan.featured ? 'bi-chat-dots' : plan.rawPrice === 5 ? 'bi-rocket-takeoff' : 'bi-cart-check']"></i>
    ...
    <div class="storage-cards-wrap">
      ...
      <div class="storage-card">
        ...
        <div class="storage-note">1 GB × $40 COP/GB</div>
...
onMounted(() => {
  ...
  updateScreenWidth()
  nextTick(() => {
    planViewport.value?.addEventListener('wheel', (e) => {
      e.preventDefault()
      if (e.deltaY > 0) nextPlan(); else prevPlan()
    }, { passive: false })
...
@media (max-width: 480px) {
  ...
  .storage-cards-wrap .storage-card {
    width: 80%;
    margin: 0 auto;
  }
}

.storage-card--variable {
  min-height: 320px;
  height: auto;
}
