  ...
  <section class="pricing">
    ...
    <div class="carousel-container">
      ...
      <div class="carousel-viewport" ref="planViewport"
        ...
        <div class="plans-track" :style="{ transform: `translateX(${planTrackOffset}px)` }">
          <div v-for="(plan, i) in plans" :key="plan.name" class="plan-card"
            :class="{ featured: plan.featured, 'plan-card--centered': i === currentPlanIndex }">
            <div v-if="plan.featured" class="badge">
            ...
            <!-- Todos los planes → abre modal de pago -->
            <button
    ...
    <div class="storage-cards-wrap">
      ...
      <div class="storage-card">
        ...
        <button class="btn-storage"><i class="bi bi-plus-circle"></i> Añadir</button>
      </div>
...
const baseWhatsAppLink = 'https://api.whatsapp.com/send/?phone=573015739461&text='
const crearMensaje = (planName) => encodeURIComponent(`Hola, estoy interesado en el ${planName}. Por favor solicito más información sobre este plan de servicio.`)

const plans = [
  { name: 'Plan Basic – Integral', icon: 'bi-gift',           price: '$5',   rawPrice: 5,   whatsappLink: baseWhatsAppLink + crearMensaje('Plan Basic – Integral'), features: ['2 usuarios', '5 GB almacenamiento', 'Todas las funciones activas', 'Soporte básico'],        ideal: 'Ideal para talleres pequeños que están empezando.', featured: false },
  { name: 'Plan Start',            icon: 'bi-stars',          price: '$19',  rawPrice: 19,  whatsappLink: baseWhatsAppLink + crearMensaje('Plan Start'),            features: ['Hasta 5 usuarios', '50 GB almacenamiento', 'Todas las funciones'],                          ideal: 'Ideal para centros pequeños con flujo moderado.',   featured: false },
  { name: 'Plan Growth',           icon: 'bi-graph-up-arrow', price: '$49',  rawPrice: 49,  whatsappLink: baseWhatsAppLink + crearMensaje('Plan Growth'),           features: ['Hasta 15 usuarios', '200 GB almacenamiento', 'Todas las funciones'],                        ideal: 'Ideal para centros medianos con varios técnicos.',  featured: false },
  { name: 'Plan Business',         icon: 'bi-briefcase-fill', price: '$99',  rawPrice: 99,  whatsappLink: baseWhatsAppLink + crearMensaje('Plan Business'),         features: ['Hasta 40 usuarios', '1 TB almacenamiento', 'Todas las funciones'],                          ideal: 'Ideal para empresas con varias sedes.',             featured: false },
  { name: 'Plan Enterprise',       icon: 'bi-building',       price: '$179', rawPrice: 179, whatsappLink: baseWhatsAppLink + crearMensaje('Plan Enterprise'),       features: ['Hasta 100 usuarios', '2 TB almacenamiento', 'Todas las funciones', 'Soporte prioritario'], ideal: 'Ideal para redes grandes de servicio técnico.',     featured: true  },
]
...
.plans-track { display: flex; gap: 32px; transition: transform 0.4s ease-in-out; will-change: transform; align-items: center; height: 40rem; width: fit-content; }
...
.plan-card { flex-shrink: 0; width: v-bind(planCardWidth + 'px'); background: var(--bg-card); border: 1px solid var(--border-soft); border-radius: 1.5rem; padding: 1rem; position: relative; cursor: pointer; min-height: 600px; display: flex; flex-direction: column; justify-content: space-between; transition: border-color 0.2s, box-shadow 0.2s, transform 0.2s, background 0.35s; }
@media (max-width: 768px) { .plan-card { width: 260px; min-height: 580px; } }
@media (max-width: 480px) { .plan-card { width: 240px; min-height: 560px; } }
@media (max-width: 360px) { .plan-card { width: 220px; min-height: 540px; } }
.plan-card--centered { border: 2px solid var(--accent); box-shadow: 0 0 0 2px var(--accent), 0 12px 40px var(--accent-glow); transform: scale(1.02); z-index: 2; }
.plan-card.featured { border: 2px solid var(--accent); background: linear-gradient(145deg, var(--bg-card), var(--bg-alt)); box-shadow: 0 0 28px var(--accent-glow); }
.plan-card--centered.featured { box-shadow: 0 0 0 2px var(--accent), 0 12px 40px var(--accent-glow); }

.badge { position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: var(--accent); color: var(--accent-text); font-size: 0.7rem; font-weight: 700; padding: 0.25rem 1rem; border-radius: 9999px; white-space: nowrap; letter-spacing: 0.5px; display: flex; align-items: center; gap: 0.25rem; z-index: 3; }
